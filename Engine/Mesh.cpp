...
void Mesh::CreateVRSImage(UINT width, UINT height)
{
	...
	vrsDesc.Width = width / 16;   // VRS ∏ ¿∫ 16x16 ∫Ì∑œ ¥‹¿ß
...
void Mesh::UploadVRSData()
{
	...
	// øπ¡¶: »≠∏È¿« ø¿∏•¬  ¿˝π›¿∫ 2x2 ∫Ì∑œ ªÁøÎ
	for (UINT y = 0; y < height; y++)
	{
		for (UINT x = 0; x < width; x++)
		{
			if (x < width / 2)
				vrsData[y * width + x] = D3D12_SHADING_RATE_1X1;  // ±‚∫ª ºŒ¿Ãµ˘ ∑π¿Ã∆Æ
			else
				vrsData[y * width + x] = D3D12_SHADING_RATE_2X2;  // 2x2 ∫Ì∑œ
		}
	}

	// æ˜∑ŒµÂ πˆ∆€ø° µ•¿Ã≈Õ ∫πªÁ
	void* mappedMemory;
...
void Mesh::Render(uint32 instanceCount, uint32 idx)
{
	...
	{
		// RSSetShadingRateImage¥¬ ID3D12GraphicsCommandList5ø°º≠∏∏ ¡¶∞¯µ«π«∑Œ, QueryInterface∑Œ »πµÊ
...
shared_ptr<Mesh> Mesh::CreateFromFBX(const FbxMeshInfo* meshInfo, FBXLoader& loader)
{
	...
	for (const vector<uint32>& buffer : meshInfo->indices)
	{
		if (buffer.empty())
		{
			// FBX ?åÏùº???¥ÏÉÅ?òÎã§. IndexBufferÍ∞Ä ?ÜÏúºÎ©??êÎü¨ ?òÎãàÍπ??ÑÏãú Ï≤òÎ¶¨
...
void Mesh::CreateVertexBuffer(const vector<Vertex>& buffer)
{
	...
	_vertexBufferView.StrideInBytes = sizeof(Vertex); // ?ïÏ†ê 1Í∞??¨Í∏∞
	_vertexBufferView.SizeInBytes = bufferSize; // Î≤ÑÌçº???¨Í∏∞	
}
...
#pragma region SkinData
	if (IsAnimMesh())
	{
		...
		// OffsetMatrix StructuredBuffer «“¥Á
